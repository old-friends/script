...
docker run --name ${NAME} -it -d -p ${PORT}:80 -d --privileged=true ${IMAGE_NAME}:latest bash -c "/usr/sbin/init" > /dev/null
...
echo -e "=-=-=-=-=-=-=-=-=-=-=-=-="
echo -e "If this URL is not working then get the IP of your interface and run the below command"
echo -e "http://<Interface IP>:${NODE_PORT}"
echo -e "=-=-=-=-=-=-=-=-=-=-=-=-="
